...
export default function HandleData({data} : HandleDataProps){
    ...
    return(
        <div className="space-y-6 text-white">
            <p className="text-5xl text-center capitalize font-black">{data.handle}</p>
            ...
            <div className="mt-20 flex flex-col gap-6">
                {links.length ? 
                    links.map(link=>(
                        <a 
                        key={link.name}
                        className="bg-white px-5 py-2 flex items-center gap-5 rounded-lg text-bl"
                        href={link.url}
                        target="_blank"
                        rel="noreferrer noopener">
                            <img src={`/social/icon_${link.name}.svg`} alt="Icono Red Social" className="w-12"/>
                            <p className="text-black capitalize font-bold text-lg">Visita mi link: {link.name}</p>
                        </a>
                    ))
